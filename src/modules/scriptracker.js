 ...
 */
export var ScripTracker = function() {
	...
	this.audioScriptNode     = this.audioContext.createScriptProcessor(this.bufferSize, 1, 2);
	...
	for (var c = 0; c < this.module.channels; c ++) {
		...
		if (this.currentTick === 0) {
			...
			if (effect !== Effects.NONE) {
				this.dispatchEvent(ScripTracker.Events.effect, this, c, registers.instrument, note, effect, effectParam);
			}
		}
 ...
 */
ScripTracker.prototype.loadModule = function(url) {
...
ScripTracker.prototype.dispatchEvent = function (event, player, channel, instrument, note, effect, effectParam) {
	var handlers = this.eventHandlers[event];
